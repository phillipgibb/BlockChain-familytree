...
export default class FamilyTreeWrapper {
  ...
  initialize(){
    let source = fs.readFileSync("FamilyTree.json");
    let contract = JSON.parse(source)["FamilyTree"];
    
    familyTreeABI = JSON.parse(contracts.abi);
    
    familyTreeByteCode = contracts.unlinked_binary;
    console.log("familyTreeByteCode = " + familyTreeByteCode)
  ...
  // compileContract(){
  //   var source = fs.readFileSync('./contracts/FamilyTree.sol',{encoding: 'utf8'}).toString().replace(/\n/g,' ');
    
  //   const compiledCode = solc.compile(code,1)
  //   const errors = []
  //   const warnings = []
  ...
  //   var familyTreeCode = compiledCode.contracts['FamilyTree'].bytecode
  //    console.log('byteCode', familyTreeCode);
    
  //   var familyTreeABI = JSON.parse(compiledCode.contracts['FamilyTree'].interface)
  //   console.log(familyTreeABI);
  //   Pudding.save(contract_data, './FamilyTree.sol.js').then(function() {
  //     console.log('File '+'./FamilyTree.sol.js'+' was created with the JS contract!' );
  //   });
  // }
